...
const [lugar, setLugar] = useState('CITT');
...
<div className="text-center text-4xl bg-amber-300 p-2">Seleccionar Horario</div>
    ...
    {listaHorarios.map((horario)=>(
        <li className="w-full duoc-item" >
            <input type='radio' defaultChecked={true} name='horarios' id={horario} className='miracaezon' onClick={()=>setDia('sdsd')}/>
            <label htmlFor={horario} onClick={()=>{
...
<input type='radio' defaultChecked={true} name='pedo' id='wena' className='miracaezon' onClick={()=>setDia('sdsd')}/>
<label htmlFor="wena" className='ellabel border-2 p-2 rounded-full'>wena ql</label>

<input type='radio'  name='pedo'/>
